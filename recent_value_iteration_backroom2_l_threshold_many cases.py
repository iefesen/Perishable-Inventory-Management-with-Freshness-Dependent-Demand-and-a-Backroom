...
"""
Created on Mon Jul  1 16:15:38 2024
                ...
                for s in s_range:
                    for lambda_c in lambda_c_range:
                        for P_b in P_b_range:
                            ...
                            row = [K, I, L, p, s, lambda_c, P_b, opt_average] + averages
                            
                            results.append(row)
                            print(case)


columns = ['K', 'I', 'L', 'p', 's', 'lambda_c', 'P_b', 'opt_average'] + [f'average_{l_t}' for l_t in range(L + 1)]
results_df = pd.DataFrame(results, columns=columns)
results_df.to_excel('average_rewards_threshold_l_analysis_for_K.xlsx', index=False)
