# -*- coding: utf-8 -*-
"""
Created on Tue Nov 12 10:08:03 2024
                                ...
                                for l in range(L+1):
                                    for l_b in range(L+1):
                                        if l == 0 or i == 0 or l_b == 0:
                                            ...
                                            immediate_rew = R[i, l, l_b, 1] - R[i, l, l_b, 0]
                                            salvage_rew = s*(max(0, I - lambda_c*l_b/L) - max(0, i - lambda_c*l/L))
                                            sales_rew = p*sum((j*lambda_c)/L for j in range(l + 1, l_b - 1))
                                            backroom_rew = p*lambda_c*(L - l_b + P_b)/L
                                            total_diff = immediate_rew
                                            if  total_diff > 0:
                                                heuristic_policy[i, l, l_b] = 1
...
df.to_excel("myopic_policy.xlsx", index=False)
