                ...
                for s in s_range:
                    for lambda_c in lambda_c_range:
                        for P_b in P_b_range:
                            ...
                            # Reward array
                            R_neg = np.zeros((I + 1, L + 1, num_actions))
                            for i in range(I + 1):
                                for l in range(L + 1):
                                    for a in range(num_actions):
                                        if a == 1:
                                            R_neg[i, l, a] = - K + s * i + p * (sum(k * arrival_probability(l_b, k) for k in range(I))) + arr_greater_than(L, I - 1) * I * p
                                        else:
                                            R_neg[i, l, a] = p * (sum(k * arrival_probability(l, k) for k in range(i))) + arr_greater_than(l, i - 1) * i * p
                            print('Rewards completed')
                            # Transition probability matrix
                            P_neg = np.zeros((I + 1, L + 1, num_actions, I + 1, L + 1))
                            for i in range(I + 1):
                                for l in range(L + 1):
                                            ...
                                            if i == 0 and l == 0:
                                                P_neg[i, l, 0, i, l] = 1
                                                for k in range(I+1):    
                                                    P_neg[i, l, 1, I - k, L - 1] = arrival_probability(L, k)
                                                P_neg[i, l, 1, 0, L] = arr_greater_than(L, I - 1)
                                            elif i == 0 and l != 0:
                                                P_neg[i, l, 0, i, l - 1] = 1
                                                for k in range(I+1):
                                                    P_neg[i, l, 1, I - k, L - 1] = arrival_probability(L, k)
                                                P_neg[i, l, 1, 0, L - 1] = arr_greater_than(L, I - 1)
                                            elif i != 0 and l == 0:
                                                P_neg[i, l, 0, i, l] = 1
                                                for k in range(I+1):
                                                    P_neg[i, l, 1, I - k, L - 1] = arrival_probability(L, k)
                                                ...
                                                for k in range(i+1):
                                                    P_neg[i, l, 0, i - k, l - 1] = arrival_probability(l, k)
                                                P_neg[i, l, 0, 0, l - 1] = arr_greater_than(l, i - 1)
                                                
                            print('Probabilities completed') 
                            
                            V_neg = np.zeros((I+1, L+1))
                            V_actions_neg = np.zeros((I + 1, L + 1, num_actions))
                            
                            # Value iteration
                            iteration = 0
                            while True:
                                V_prev_neg = np.copy(V_neg)
                                iteration += 1
                                for i in range(I + 1):
                                    for l in range(L + 1):
                                        ...
                                        for a in range(num_actions):
                                            if (i == 0 or l == 0) and a == 0:
                                                V_actions_neg[i, l, a] = -float('inf')
                                            else:
                                                V_actions_neg[i, l, a] = R_neg[i, l, a] + gamma * np.sum(P_neg[i, l, a] * V_prev_neg)
                                                
                                V_neg = np.max(V_actions_neg, axis = 2)
                                
                                if np.max(np.abs(V_neg - V_prev_neg)) < eps:
                                    break
                            
                            neglecting_policy = np.argmax(V_actions_neg, axis = 2)
                            end_time = time.time()
                            
                            stationary_dist = calculate_stationary_distribution(P, neglecting_policy, I, L)
                            
                            neglecting_avg_reward = 0
                            for i in range(I+1):
                                for l in range(L+1):
                                    for l_b in range(L+1):
                                        if l >= l_b:
                                            ...
                                            neglecting_avg_reward += R[i, l, l_b, neglecting_policy[i, l]] * stationary_dist[i, l, l_b]
                            end_time = time.time()
                            ...
                            results.append({
                                ...
                                "lambda_c": lambda_c,
                                "P_b": P_b,
                                "Neglecting policy": neglecting_avg_reward,
                                "Time neglecting policy": total_time,
                            })
...
df.to_excel("assuming_zero_backroom_decay3.xlsx", index=False)
