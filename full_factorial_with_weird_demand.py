# -*- coding: utf-8 -*-
"""
Created on Mon Sep 16 13:17:29 2024
...
def arrival_probability(L_i, k):
    if L_i < L / 2:
        return 1.0 if k == 0 else 0.0  # Only 0 arrivals are possible
    arr = lambda_c * (L_i / L)
    return poisson.pmf(k, arr)

def arr_greater_than(L_i, k):
    if L_i < L / 2:
        return 0.0  # No arrivals greater than 0 are possible
    arr = lambda_c * (L_i / L)
                ...
                for s in s_range:
                    for lambda_c in lambda_c_range:
                        for P_b in P_b_range:
                            ...
                            regression_i = df.iloc[case - 1, 7]
                            regression_l = df.iloc[case - 1, 8]
                            
                            
                            regression_l_average = calculate_avg_reward_for_threshold_l(regression_l, I, L, P, R)
                            regression_i_average = calculate_avg_reward_for_threshold_i(regression_i, I, L, P, R)
                            # Record results
                            results.append({
                                "K": K,
                                "I": I,
                                ...
                                'Regression_i_average': regression_i_average,
                                'Regression_l_average': regression_l_average
                            })
...
df.to_excel("master_file_weirs_demand.xlsx", index=False)
